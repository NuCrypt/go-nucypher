...
// Arguments are used for passing query parameters
...
// NewClient creates a new NuCrypt client with the provided options.
...
func (c *Client) get(url string, target interface{}) error {
	...
	if resp.StatusCode != http.StatusOK {
		return errors.New("error from nucypher api")
	}
...
// save returns an error if the http client cannot save the request to nucypher..
func (c *Client) save(httpMethod string, url string, payload interface{}, target interface{}) error {
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
		return errors.New(fmt.Sprintf("error from nucypher api. httpCode: %d, response: %s", resp.StatusCode, b))
	}
